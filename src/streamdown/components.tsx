...
const CodeComponent = ({
	...
	if (inline) {
		return (
			<code
				className={cn('rounded bg-gray-100 px-1.5 py-0.5 font-mono text-sm', className)}
	...
	return (
		<CodeBlock
			...
			language={language}
			preClassName="overflow-x-auto font-mono text-xs p-4 bg-gray-100/40"
		>
			<CodeBlockDownloadButton code={code} language={language} />
			<CodeBlockCopyButton />
		</CodeBlock>
...
export const components: Options['components'] = {
	ol: ({ node, children, className, ...props }) => (
		<ol
			className={cn('ml-4 list-outside list-decimal whitespace-normal', className)}
	...
	li: ({ node, children, className, ...props }) => (
		<li className={cn('py-1', className)} data-streamdown="list-item" {...props}>
			{children}
		</li>
	),
	...
	hr: ({ node, className, ...props }) => (
		<hr
			className={cn('my-6 border-gray-200', className)}
	...
	a: ({ node, children, className, href, ...props }) => {
		...
		return (
			<a
				className={cn('font-medium text-blue-600 underline', className)}
	...
	thead: ({ node, children, className, ...props }) => (
		<thead className={cn('bg-gray-100/80', className)} data-streamdown="table-header" {...props}>
			{children}
		</thead>
	),
	tbody: ({ node, children, className, ...props }) => (
		<tbody
			className={cn('divide-y divide-gray-200 bg-gray-100/40', className)}
	...
	td: ({ node, children, className, ...props }) => (
		<td className={cn('px-4 py-2 text-sm', className)} data-streamdown="table-cell" {...props}>
			{children}
		</td>
	),
	blockquote: ({ node, children, className, ...props }) => (
		<blockquote
			className={cn('my-4 border-l-4 border-gray-600/30 pl-4 text-gray-600 italic', className)}
	...
	sup: ({ node, children, className, ...props }) => (
		<sup className={cn('text-sm', className)} data-streamdown="superscript" {...props}>
			{children}
		</sup>
	),
	sub: ({ node, children, className, ...props }) => (
		<sub className={cn('text-sm', className)} data-streamdown="subscript" {...props}>
			{children}
		</sub>
	)
};
