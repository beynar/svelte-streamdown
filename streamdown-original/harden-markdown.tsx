...
export default function hardenReactMarkdown<TMarkdownComponent extends ComponentType<Options>>(
	...
	return function HardenedReactMarkdown(
		...
		const transformUrl = (url: unknown, allowedPrefixes: string[]): string | null => {
			if (!url) return null;
		...
		const hardenedComponents: Components = {
			a: ({ href, children, node, ...props }) => {
				...
				if (transformedUrl !== null) {
					...
					return (
						<a href={transformedUrl} {...props} target="_blank" rel="noopener noreferrer">
							{children}
						</a>
					);
				}
			...
			img: ({ src, alt, node, ...props }) => {
				...
				return (
					<span className="inline-block rounded bg-gray-200 px-3 py-1 text-sm text-gray-600 dark:bg-gray-700 dark:text-gray-400">
